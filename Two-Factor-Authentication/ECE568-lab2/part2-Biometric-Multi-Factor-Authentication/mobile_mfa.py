...
class BioConnect:
	...
	def getAuthenticatorStatus(self):
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply to get Authenticator status (possibly due to time limit)")
		
		if reply.get("status","")=="active" and \
			(reply.get("face_status","")=="enrolled" or reply.get("voice_status","")=="enrolled" \
			or reply.get("fingerprint_status","")=="enrolled" or reply.get("eye_status","")=="enrolled"):
			return "active"
	...
	def sendStepup(self,
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply to send Stepup")
	...
	def getStepupStatus(self):
		...
		try:
			# Parse the JSON reply
			reply = json.loads(result.content.decode('utf-8'))
			return reply.get("user_verification","{}").get("status","")
